...
@section('content')
    <div class="grid lg:grid-cols-3 gap-4">
        ...
        <div>
            <form action="{{ route('jadwal.update', $transaksi) }}" method="post"
                ...
                <div class="card bg-base-100 shadow-xl">
                    <div class="card-body">
                        ...
                        <label class="form-control w-full max-w-xs">
                            ...
                            <select name="hari" class="select select-bordered w-full max-w-xs">
                                <option disabled selected>Pilih</option>
                                @foreach(\App\Enums\HariJadwalEnum::cases() as $row)
                                    <option
                                        @selected($transaksi->hari == $row->value) value="{{ $row->value }}">{{ $row->value }}</option>
                                @endforeach
                            </select>
                            @error('hari')
                        ...
                        <label class="form-control w-full max-w-xs">
                            ...
                            <select name="range_jam" class="select select-bordered w-full max-w-xs">
                                <option disabled selected>Pilih</option>
                                @foreach(\App\Enums\JamJadwalEnum::cases() as $row)
                                    <option
                                        @selected($transaksi->range_jam == $row->value) value="{{ $row->value }}">{{ $row->value }}</option>
                                @endforeach
                            </select>
                            @error('range_jam')
                            <small class="text-error">{{ $message }}</small>
                        ...
                        <label class="form-control w-full max-w-xs">
                            ...
                            <select name="instruktur_id" class="select select-bordered w-full max-w-xs">
                                ...
                                @foreach($instruktur as $row)
                                    <option
                                        @selected($transaksi->instruktur_id== $row->id) value="{{ $row->id }}">{{ $row->name }}</option>
                                @endforeach
                            </select>
                            @error('instruktur_id')
                            <small class="text-error">{{ $message }}</small>
